...
| Flag | Effect |
|------|--------|
| `--model <alias>` | Model selection: `sonnet`, `opus`, `haiku`, `fable`, or full name like `claude-fable-5`. Fable 5 is Mythos-class (above Opus), $50/M output tokens, designed for multi-day autonomous work. See `references/fable-5-model.md` |
...
## Frontier Model Prompt Patterns

When delegating to Mythos-class models (Fable 5) or other high-capability reasoning models, prompt style matters more than with weaker models. These models perform best when given **intentions, not instructions**.

### The Lean Prompt Rule
- **Give the hard requirements only** — what must exist, what must work. Everything else is creative authority.
- **Don't tell it how** — layout, colors, typography, CTA placement, trust signals, pricing presentation, image treatment. The model is smarter at design decisions than the constraints you'd impose.
- **Don't list anti-patterns** — a model scoring 80%+ on SWE-Bench Pro does not need to be told that forms should be accessible or that testimonials convert better than equipment photos. It knows this. Telling it anyway wastes tokens and signals distrust.
- **Don't enumerate "DO NOT replicate X, Y, Z"** — the model can study the existing site and decide what to keep vs elevate. Pre-judging what's wrong constrains its creative range.

### The Pre-Research Pattern
For complex delegation tasks, do the environment research yourself BEFORE handing off:
1. Research the existing site/brand/codebase to understand the landscape
2. Extract only the hard constraints (must-haves, must-not-breaks)
3. Craft a ~150-250 word prompt with: what to study, what to build, hard constraints
4. Let the model handle all creative and technical decisions

This saves the user's output tokens ($50/M for Fable 5) by preventing the model from wasting turns on research you already did.

### Example — Good vs Bad
**Bad** (over-constrained, ~600 words):
> "Build a website. Use dark theme. The hero should have the logo centered at 200px with the tagline below in 18px white text. Services section should use 3-column grid with border-radius 8px cards. Include anti-spam honeypot. No reCAPTCHA — it clashes visually. Use Google Fonts pairing of Playfair Display + Inter. DO NOT replicate the loading spinner from the old site. DO NOT use system default fonts. The phone field must be type=tel not type=number..."

**Good** (lean, ~150 words):
> "Build a website for [business]. First study our existing site at [URL] and our Instagram [@handle] to understand the brand. Then build: static HTML/CSS/JS, Netlify deploy target, booking form with [fields], GA4 + Ads placeholders. Mobile-first. Beyond those constraints, you have full creative authority."
